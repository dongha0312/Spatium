...
- **SpatiumTests** — 유닛 테스트 63개 (Swift Testing): 백엔드 API 계약과 파일 기반 multipart 구성, 에디터 undo·redo/draft, SceneKit 증분 렌더링·모델 템플릿 LRU·뷰어 리소스 해제, 스캔 생명주기, GLB 보정 스트리밍·가구 치수·프로필 이미지 다운샘플링·변환, 사용자 가구 동시 저장·파일 롤백, 설정 캐시 작업 범위, 로컬 캐시 저장 실패 복구
