<div align="center">
  <img src="Spatium/Assets.xcassets/SpatiumCube.imageset/spatium_cube.png" width="112" alt="Spatium 로고">
...
- 📷 **방 스캔** — RoomPlan(LiDAR)으로 방을 스캔해 USDZ + 메타데이터로 저장, 방 하나 단위 스캔 안내(준비 시트 + 스캔 중 배너)
...
- 👋 **온보딩** — 첫 실행 시 실제 앱 화면 4장으로 핵심 흐름(스캔 → 편집 → 가구 생성 → 동기화) 소개
...
| 스캔 | 방 단위 스캔 준비 안내 → RoomPlan 캡처 → 리뷰 → 파일 공유 / 서버 업로드(로그인 필요) |
...
프로젝트·방 목록 캐시의 초기 읽기·JSON 디코딩과 atomic 파일 쓰기·로그아웃 삭제도 utility 우선순위의 전용 직렬 큐에서 처리합니다. 로그인 상태에서는 캐시를 먼저 불러온 뒤 서버 새로고침을 이어가며, 로드 중 로그아웃이나 새 저장이 발생하면 이전 파일 스냅샷을 적용하지 않습니다. 각 스냅샷에 증가하는 revision을 붙여 연속 변경 작업이 겹쳐도 오래된 저장이 최신 `projects.json`을 덮어쓰지 않으며, 프로젝트별 방 개수와 방별 항목 수는 병렬로 집계하되 순간 동시 요청 수를 최대 5개로 제한(슬라이딩 윈도)해 데이터 규모가 커져도 요청이 폭증하지 않으며, 집계가 끝난 뒤 전체 캐시를 각각 한 번만 기록합니다. 로그인 화면의 게스트 프로젝트 유실 위험 계산도 백그라운드에서 실행하고 확인이 끝날 때까지만 로그인 동작을 보류합니다. 저장 실패 알림과 최신 메모리 상태 재시도, 게스트의 로컬 프로젝트·방 작성 및 로그인 전 경고 동작은 그대로 유지합니다.
...
측정 모드는 웹 프런트엔드와 같은 방식입니다. 바닥 mesh 삼각형을 높이별로 그룹핑해 가장 넓은 그룹을 메인 바닥으로 선택하고, 그 외곽 edge(사용 횟수 1인 변)를 실제 방 모양 그대로 치수선(선 + 양끝 눈금 + cm 라벨)으로 표시합니다. ㄱ자 방도 각 벽면별 길이가 정확히 나오며, 방 높이선과 함께 실제 바닥 폴리곤 면적을 "면적 xx.xx m² x.x 평" 배지로 보여줍니다. 측정 중 가구를 선택하면 해당 가구의 가로·세로·높이 라벨도 표시됩니다. 바닥 mesh가 없는 박스 방은 bounding box 직사각형으로 폴백합니다.

3D 에디터의 벽·바닥 색상 팝오버에는 웹 프런트엔드와 동일한 "기본 색상으로 되돌리기" 스와치가 있습니다. 색을 고르지 않은 스캔 방은 벽·바닥을 기본색으로 덮지 않고 스캔 원본 재질을 그대로 보여주며, 색을 칠했다가 해제하면 처음 기록해둔 원본 diffuse로 복원됩니다. (박스 방은 기본색 유지)
...
- **SpatiumTests** — 유닛 테스트 82개 (Swift Testing): 바닥 폴리곤 기반 방 치수·면적·외곽선 계산(웹 calculateRoomMeasurements 대응), 백엔드 API 계약과 파일 기반 multipart 구성(메모리 multipart의 파일 파트 거부 포함), 에디터 undo·redo와 복구본의 백그라운드 읽기·저장·삭제·revision 순서, SceneKit 증분 렌더링·모델 템플릿 LRU·뷰어 리소스 해제, 벽·바닥 색 선택 해제 시 스캔 원본 재질 복원, 서버 룸 USDZ의 백그라운드 Base64 복원·손상 응답 처리, 로컬 스캔 JSON의 백그라운드 읽기·디코딩·항목 집계·캐시 삭제, 개수 집계의 동시 요청 상한(슬라이딩 윈도), 스캔 생명주기, GLB 보정 스트리밍·가구 치수·프로필 이미지 다운샘플링·변환, 사용자 가구의 백그라운드 초기 로드·초기화 경합·동시 저장·파일 롤백, 설정 캐시 작업 범위, 프로젝트 캐시의 백그라운드 초기 읽기·저장·최신 스냅샷 순서·실패 복구·게스트 위험 계산, 홈 반복 애니메이션 실행 조건(탭 표시·포그라운드·Reduce Motion)
