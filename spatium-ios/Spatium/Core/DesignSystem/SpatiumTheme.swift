...
enum SpatiumTheme {
    ...
    /// 상단 Glass가 스크롤 콘텐츠의 강한 색을 그대로 받아 급격히 변하지 않도록 하는 중성 틴트.
    /// 투명도는 유지하면서 라이트·다크 배경색 쪽으로 색 변화 폭만 낮춘다.
    static let stableHeaderGlassTint = adaptive(
        light: "#F2EEE6",
        dark: "#202829",
        lightAlpha: 0.30,
        darkAlpha: 0.36
...
private struct SpatiumChromeGlassModifier: ViewModifier {
    ...
    let stabilizesBackgroundColor: Bool
    ...
    func body(content: Content) -> some View {
        if #available(iOS 26.0, *) {
            content
                .glassEffect(
                    .regular.tint(
                        stabilizesBackgroundColor
                            ? SpatiumTheme.stableHeaderGlassTint
                            : SpatiumTheme.accent.opacity(0.08)
                    ),
...
extension View {
    ...
    func spatiumChromeGlass(
        cornerRadius: CGFloat,
        stabilizesBackgroundColor: Bool = false
    ) -> some View {
        modifier(
            SpatiumChromeGlassModifier(
                cornerRadius: cornerRadius,
                stabilizesBackgroundColor: stabilizesBackgroundColor
            )
        )
    }
}
